...
旧站提到的常用传感器包括 DHT11（温湿度）、BH1750（光照）、HC-SR04（超声波测距）。

## 串口数据结构截图

<Figure
  src="/images/notes/uart-data-structure.png"
  alt="串口数据结构与传感器字段示意截图"
  caption="旧站保留的串口数据结构与传感器字段示意截图，适合用于数据采集与网关说明。"
/>

## 网关代码片段

下面代码来自旧站保留的实现片段，用于说明 Linux 串口读取、数据结构解析和控制命令发送之间的关系。本阶段没有在当前环境编译或验证该代码，也不声称它是完整可直接部署的程序。
...
- 旧代码是否是完整程序仍需人工确认。
- 代码依赖的串口设备、头文件和网络接收部分需要结合原实验环境核查。

<Callout type="info" title="相关项目">
本篇中的串口通信、数据结构解析和控制命令发送关系，已作为 [智能家居 IoT 系统](/projects/smart-home-iot) 项目案例的技术背景使用；这不代表旧代码已经在当前工程环境中重新编译验证。
</Callout>
